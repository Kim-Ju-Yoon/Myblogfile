...
public class ReplyController {
	...
	// 댓글 목록 리스트
	@RequestMapping(value = "/replies/{uid}/{page}", method = RequestMethod.GET)
	public ResponseEntity <ReplyPageVO> getList(@PathVariable("page") int page,@PathVariable("uid") int uid){
		System.out.println(uid);
		CriteriaVO cri = new CriteriaVO(page,10);
		return new ResponseEntity<>(rs.list(cri, uid),HttpStatus.OK);
	}
	// 댓글 수정
